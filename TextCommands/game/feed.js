...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        let soLuongAnimal = await client.xemAnimal(author, checkAnimal(nameAnimal)); // check số animal chưa cho ăn trong db
        let dataFed = await feedAnimalModel.findOne({ userId: author, name: checkAnimal(nameAnimal) });
        if (!dataFed) {
            dataFed = new feedAnimalModel({
                userId: author, 
                name: checkAnimal(nameAnimal)
            })
        }
        ...
        if (soLuongAnimal == 0) return message.reply(`Bạn đã cho ${checkAnimal(nameAnimal)} ăn đầy đủ`);

        let foodEmoji = {
            "lúa" : "cây lúa <:LC_Wheat:1155701062670504037>",
            "hạt lúa" : "hạt lúa <:seeds97:1155701097806180372>",
        };
		let soLuongLua = await client.nongSan(author, "lúa");
		let soLuongHatLua = await client.nongSan(author, "hạt lúa");
		if (nameAnimal == 'bo' && soLuongLua < soLuong) return message.reply('Bạn không đủ lúa cho bò ăn <:LC_Wheat:1155701062670504037>');
		if (nameAnimal == 'ga' && soLuongHatLua < soLuong) return message.reply('Bạn không đủ hạt lúa cho gà ăn <:seeds97:1155701097806180372>');
        await client.choAn(author, checkAnimal(nameAnimal), soLuong);
        await client.truNongSan(author, checkFood(nameAnimal), soLuong);
        await message.reply(`Bạn đã cho **${soLuong} ${checkAnimal(nameAnimal)} ${animalEmoji[checkAnimal(nameAnimal)]}** ăn thành công **${foodEmoji[checkFood(nameAnimal)]}**`);
